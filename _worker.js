...
const GLOBAL_CONFIG = {
  // 缓存策略配置
  CACHE_POLICIES: {
    SUBSCRIPTION_CLASH: 7200,    // Clash订阅缓存时间（秒）
    SUBSCRIPTION_SINGBOX: 7200,  // Singbox订阅缓存时间（秒）
    SUBSCRIPTION_BASE64: 3600,   // Base64订阅缓存时间（秒）
    PROXY_IP_LIST: 14400,        // IP列表缓存时间（秒）
    NODE_HEALTH: 300             // 节点健康状态缓存时间（秒）
  },
  // 请求超时设置
  REQUEST_TIMEOUT: {
    DEFAULT: 30000,              // 默认请求超时（毫秒）
    PROXY_CONNECT: 15000,        // 连接超时（毫秒）
    HEALTH_CHECK: 5000           // 健康检查超时（毫秒）
  },
  // 性能优化配置
  PERFORMANCE: {
...
class CacheManager {
  ...
  // 预缓存常用资源
  async preCacheCommonResources() {
    // 预缓存常用资源的逻辑
    console.log('Pre-caching common resources...');
  }
}
...
class NodeManager {
  ...
  // 获取最佳节点
  getBestNodes(filters = {}, count = 20) {
    let allNodes = [];
    
    // 合并所有节点
    if (this.proxyIPPool.length > 0) {
      allNodes = allNodes.concat(this.proxyIPPool.map(ip => ({ type: 'proxy', value: ip })));
    }
    if (this.socks5Pool.length > 0) {
      allNodes = allNodes.concat(this.socks5Pool.map(socks => ({ type: 'socks5', value: socks })));
    }
    if (this.httpPool.length > 0) {
      allNodes = allNodes.concat(this.httpPool.map(http => ({ type: 'http', value: http })));
    }

    // 按地区过滤
    if (filters.region) {
      allNodes = allNodes.filter(node => this.getNodeRegion(node) === filters.region);
    } else if (filters.clientCountry) {
      const recommendedRegions = this.getRecommendedRegions(filters.clientCountry);
      allNodes = allNodes.filter(node => recommendedRegions.includes(this.getNodeRegion(node)));
    }

    // 执行健康检查
    if (GLOBAL_CONFIG.HEALTH_CHECK.ENABLED) {
      this.performHealthChecks(allNodes);
    }

    // 智能排序
    return this.sortNodesBySmartScore(allNodes, filters.sortBy || 'auto', filters.clientCountry).slice(0, count);
  }

  // 获取推荐地区
  getRecommendedRegions(clientCountry) {
    return this.geoMap[clientCountry] || this.geoMap['default'];
  }
  ...
  // 按智能分数排序节点
  sortNodesBySmartScore(nodes, criteria = 'auto', clientCountry = null) {
    return [...nodes].sort((a, b) => {
      const aHealth = this.nodeHealth.get(a.value) || { latency: 9999, success: 0 };
      const bHealth = this.nodeHealth.get(b.value) || { latency: 9999, success: 0 };
      
      const aScore = this.calculateNodeScore(a, aHealth, clientCountry);
      const bScore = this.calculateNodeScore(b, bHealth, clientCountry);
      
      return bScore - aScore; // 降序排列，分数越高越优先
    });
  }

  // 计算节点分数
  calculateNodeScore(node, healthData, clientCountry = null) {
    let score = 0;
    
    // 基础分数
    score += 1000 - Math.min(healthData.latency, 999);
    
    // 成功率权重
    score += (healthData.success || 0) * 100;
    
    // 地理位置加成
    if (clientCountry && node.region) {
      const recommendedRegions = this.getRecommendedRegions(clientCountry);
      const regionIndex = recommendedRegions.indexOf(node.region);
      if (regionIndex !== -1) {
        score += (recommendedRegions.length - regionIndex) * 50;
      }
    }
  ...
  async performHealthChecks(nodes) {
    const nodesToCheck = nodes.slice(0, GLOBAL_CONFIG.HEALTH_CHECK.SAMPLE_SIZE);
    const promises = nodesToCheck.map(async (node) => {
      try {
        const startTime = Date.now();
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), GLOBAL_CONFIG.REQUEST_TIMEOUT.HEALTH_CHECK);
        ...
        this.nodeHealth.set(node.value, {
          latency,
          success: 1,
          lastCheck: Date.now()
        });
...
class SubscriptionManager {
  constructor(nodeManager, cacheManager) {
  ...
  parseSubscriptionUrl(url) {
    ...
    return {
      format: params.get('clash') ? 'clash' : 
             params.get('sb') ? 'singbox' : 
             params.get('sub') ? 'base64' : 'default',
      region: params.get('region'),
      sort: params.get('sort') || 'auto',
      limit: parseInt(params.get('limit') || '20', 10),
      realtime: params.get('realtime') === 'true',
      filter: params.get('filter'),
      clientCountry: params.get('country')
    };
  }

  // 获取缓存的订阅内容
  async getCachedSubscription(cacheKey, options = {}) {
    try {
      // 如果是实时模式，不使用缓存
      if (options.realtime) {
        return await this.generateSubscription(options);
      }
      ...
      // 缓存未命中，生成新的订阅内容
      const subscriptionContent = await this.generateSubscription(options);
      
      // 根据订阅格式设置不同的缓存时间
      let ttl = 3600;
      switch (options.format) {
        case 'clash':
          ttl = GLOBAL_CONFIG.CACHE_POLICIES.SUBSCRIPTION_CLASH;
          break;
        case 'singbox':
          ttl = GLOBAL_CONFIG.CACHE_POLICIES.SUBSCRIPTION_SINGBOX;
          break;
        case 'base64':
          ttl = GLOBAL_CONFIG.CACHE_POLICIES.SUBSCRIPTION_BASE64;
          break;
      }
      
      // 存入缓存
      await this.cacheManager.set(cacheKey, subscriptionContent, ttl);
      
      return subscriptionContent;
    } catch (error) {
      console.error('Failed to get cached subscription:', error);
      // 返回备用内容
      return await this.getFallbackSubscription(options);
    }
  }

  // 生成订阅内容
  async generateSubscription(options = {}) {
    try {
      // 获取最佳节点
      const nodes = this.nodeManager.getBestNodes({
        region: options.region,
        sortBy: options.sort,
        clientCountry: options.clientCountry
      }, options.limit);

      // 根据格式生成不同的订阅内容
      switch (options.format) {
        case 'clash':
          return await this.generateClashConfig(nodes, options);
        case 'singbox':
          return await this.generateSingboxConfig(nodes, options);
        case 'base64':
          return await this.generateBase64Subscription(nodes, options);
        default:
          return await this.generateDefaultSubscription(nodes, options);
      }
    } catch (error) {
      console.error('Failed to generate subscription:', error);
      throw error;
    }
  }

  // 生成Clash配置
  async generateClashConfig(nodes, options = {}) {
    // 这里将集成到原有的Clash配置生成逻辑
    return 'Clash配置内容';
  }

  // 生成Singbox配置
  async generateSingboxConfig(nodes, options = {}) {
    // 这里将集成到原有的Singbox配置生成逻辑
    return 'Singbox配置内容';
  }

  // 生成Base64订阅
  async generateBase64Subscription(nodes, options = {}) {
    // 这里将集成到原有的Base64订阅生成逻辑
    return btoa('Base64订阅内容');
  }

  // 生成默认订阅
  async generateDefaultSubscription(nodes, options = {}) {
    // 这里将集成到原有的默认订阅生成逻辑
    return '默认订阅内容';
  }

  // 获取备用订阅内容
  async getFallbackSubscription(options = {}) {
    // 返回备用订阅内容，确保服务可用性
    return '备用订阅内容';
  }
...
class PerformanceMonitor {
  ...
  static requestsProcessed = 0;
  static totalResponseTime = 0;
  static enableLogging = true;
  
  static start(uuid, requestInfo = {}) {
    this.cache.set(uuid, {
      startTime: Date.now(),
      requestInfo,
      metrics: {}
    ...
    if (this.enableLogging) {
      console.log(`[开始请求] UUID: ${uuid}, 路径: ${requestInfo.path || 'unknown'}`);
    }
  }
  
  static end(uuid, result = {}) {
    const requestData = this.cache.get(uuid);
    if (!requestData) return;
    
    const duration = Date.now() - requestData.startTime;
    requestData.metrics.duration = duration;
    requestData.metrics.success = result.success;
    ...
    if (this.enableLogging && duration > 1000) {
      console.log(`[请求完成] UUID: ${uuid}, 耗时: ${duration}ms, 状态: ${result.success ? '成功' : '失败'}`);
    }
    
    this.cache.delete(uuid);
  }
  
  static recordMetric(uuid, name, value) {
    const requestData = this.cache.get(uuid);
    if (requestData) {
      requestData.metrics[name] = value;
    }
  }
  
  static get averageResponseTime() {
    return this.requestsProcessed > 0 ? this.totalResponseTime / this.requestsProcessed : 0;
  }
...
class ErrorHandler {
  ...
  static validateRequest(request, requiredParams = []) {
...
// 初始化订阅管理器
const subscriptionManager = new SubscriptionManager(nodeManager, cacheManager);
let proxyhostsURL;
let 请求CF反代IP = 'false';
...
export default {
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      // WebSocket请求处理
      const upgradeHeader1 = request.headers.get('Upgrade');
      if (upgradeHeader1 === 'websocket') {
        PerformanceMonitor.recordMetric(uuid, 'request_type', 'websocket');
        return handleWebSocket(request);
      }
      
      // API请求处理
      if (pathname.startsWith('/api/')) {
        PerformanceMonitor.recordMetric(uuid, 'request_type', 'api');
        return this.handleApiRequest(request, pathname, env, uuid, clientInfo);
      }
      
      // 订阅请求处理
      if (this.isSubscriptionRequest(pathname)) {
        PerformanceMonitor.recordMetric(uuid, 'request_type', 'subscription');
        return this.handleSubscriptionRequest(request, env, uuid, clientInfo);
      }
      
      // KV请求处理
      if (pathname.includes('/edit')) {
        PerformanceMonitor.recordMetric(uuid, 'request_type', 'kv');
        return KV(request, env);
      }
      
      // 最佳IP请求处理
      if (pathname.includes('/bestip')) {
        PerformanceMonitor.recordMetric(uuid, 'request_type', 'bestip');
        return bestIP(request, env);
      }
      
      // 继续处理原有逻辑
      PerformanceMonitor.recordMetric(uuid, 'request_type', 'default');
      ...
      const upgradeHeader2 = request.headers.get('Upgrade');
      if (!upgradeHeader2 || upgradeHeader2 !== 'websocket') {
      ...
      // 结束性能监控
      PerformanceMonitor.end(uuid, { success: true });
  ...
  // 收集客户端信息
  collectClientInfo(request) {
    return {
      userAgent: request.headers.get('User-Agent') || '',
      ip: request.headers.get('CF-Connecting-IP') || request.headers.get('X-Forwarded-For') || 'unknown',
      region: request.cf?.region || 'unknown',
      country: request.cf?.country || 'unknown',
      browser: this.detectBrowser(request.headers.get('User-Agent') || ''),
  ...
  initializeEnv(env) {
    // 初始化BotToken和ChatID
    BotToken = env.BOT_TOKEN || BotToken;
    ChatID = env.CHAT_ID || ChatID;
    
    // 初始化其他环境变量
    if (env.GO2SOCKS5S) {
      go2Socks5s = env.GO2SOCKS5S.split(',').filter(Boolean);
    }
    
    // 初始化性能监控配置
    if (env.PERFORMANCE_LOGGING) {
      PerformanceMonitor.enableLogging = env.PERFORMANCE_LOGGING === 'true';
    }
  },
  ...
  handleOptionsRequest() {
    return new Response('', {
      headers: {
        ...
        'Access-Control-Allow-Headers': 'Content-Type, channel, user-agent, host',
    ...
    // 判断是否为订阅请求
    isSubscriptionRequest(pathname) {
      const subscriptionPatterns = ['sub', 'clash', 'singbox', 'profile'];
      return subscriptionPatterns.some(pattern => pathname.toLowerCase().includes(pattern));
    },
    
    // 处理API请求
    async handleApiRequest(request, pathname, env, uuid, clientInfo) {
      try {
        console.log(`[API请求] UUID: ${uuid}, 路径: ${pathname}`);
        
        // 根据路径处理不同的API请求
        if (pathname === '/api/health') {
          return this.handleHealthCheck(uuid);
    ...
    handleHealthCheck(uuid) {
      const healthStatus = {
        ...
        features: {
          cacheEnabled: true,
          healthCheck: GLOBAL_CONFIG.HEALTH_CHECK.ENABLED,
    ...
    async handleStatsRequest() {
      const stats = {
        ...
        nodeHealthEntries: nodeManager.nodeHealth.size,
    ...
    // 处理订阅请求
    async handleSubscriptionRequest(request, env, uuid, clientInfo) {
      try {
        const url = new URL(request.url);
        
        // 使用订阅管理器解析参数
        const options = subscriptionManager.parseSubscriptionUrl(url);
        
        // 添加客户端信息到选项
        options.clientInfo = clientInfo;
        
        // 生成缓存键
        const cacheKey = `sub_${options.format}_${options.region || 'all'}_${options.limit}_${options.sort}`;
        
        // 获取订阅内容
        const content = await subscriptionManager.getCachedSubscription(cacheKey, options);
        
        // 设置响应头
        const headers = {
          'Content-Type': subscriptionManager.getContentTypeForFormat(options.format),
          'Access-Control-Allow-Origin': '*',
          'Content-Disposition': `attachment; filename="subscription.${options.format === 'clash' ? 'yaml' : options.format === 'singbox' ? 'json' : 'txt'}"`,
          'X-Request-ID': uuid,
          'X-Content-Size': content.length,
          'X-Node-Count': options.limit
        };
...
async function proxyUrl(proxyUrl, targetUrl) {
    const urls = await 整理(proxyUrl);
    const 完整网址 = urls[Math.floor(Math.random() * urls.length)];
...
131.0.72.0/22`;
    ...
    <div class="warning-notice">
        <h3>🚨 网络环境警告</h3>
        <p><strong>检测到您当前很可能处于非直连网络环境中！</strong></p>
        <p>在非直连状态下进行的IP优选测试结果将不准确，可能导致：</p>
        <ul>
            <li>延迟数据失真，无法反映真实网络状况</li>
            <li>优选出的IP在直连环境下表现不佳</li>
            <li>测试结果对实际使用场景参考价值有限</li>
        </ul>
        <p><strong>建议操作：</strong>请关闭所有可能影响网络连接的软件，确保处于直连网络环境后重新访问本页面。</p>
    </div>
    ` : ''}
...
async function 生成配置信息(userID, hostName, sub, UA, 请求CF反代IP, _url, fakeUserID, fakeHostName, env) {
    ...
    if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
        ...
        // 如果是使用默认域名，则改成一个workers的域名，订阅器会加上连接信息
        if (hostName.includes(".workers.dev")) {
            noTLS = 'true';
            ...
            <div class="section details-section">
                ...
                <details>
                    ...
                    <div class="details-content" id="tipsContent">
                        <p><strong>1. PassWall/PassWall2:</strong> 订阅编辑的 <strong>用户标识(User-Agent)</strong> 设置为 <strong>PassWall</strong> 即可。</p>
...
async function httpConnect(addressRemote, portRemote) {
    ...
    try {
        ...
        reader.releaseLock();
        throw new Error(`处理HTTP连接响应失败: ${err.message}`);
    }
